...
async function isParticipantInGroup(userId, groupId, participantNumber) {
  ...
  try {
    ...
    console.log(`[DEBUG][${userId}] Checking if ${participantNumber} is in group ${groupId}`);
...
async function addParticipantToGroup(userId, groupId, participantNumber) {
  ...
  try {
    ...
    console.log(`[DEBUG][${userId}] Add participant result:`, result);
    
    // Check result
    if (result && result.length > 0) {
      const participantResult = result[0];
      if (participantResult.status === '200') {
        console.log(`[DEBUG][${userId}] Successfully added ${participantNumber} to group ${groupId}`);
        return true;
      } else {
...
async function promoteParticipant(userId, groupId, participantNumber) {
  ...
  try {
    ...
    console.log(`[DEBUG][${userId}] Found participant: ${participant.id} for number ${participantNumber}`);
    ...
    if (result && result.length > 0) {
      const participantResult = result[0];
      if (participantResult.status === '200') {
        console.log(`[DEBUG][${userId}] Successfully promoted ${participantNumber} in group ${groupId}`);
        return true;
      } else {
        const errorCode = participantResult.status || 'unknown';
